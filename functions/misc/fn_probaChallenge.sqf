/* run a challenge based on success probability */
...
if (_chatty) then {
	...
	hint format ["Challenge system is up and running.\n Maximum time to run : %1 s.\nFinal probability of success : ~%2",_maxDuration, _str];
};
...
for [{_curAttempt = _totAttempts}, {(_curAttempt > 0) && !(_stopHacking)}, {_curAttempt = _curAttempt}] do {
		...
		*/
		...
		if (random 1 < _curProba) exitWith {
			if (_chatty) then {
				...
				hint format ["** LUCKY **\nChallenge succeeded after %1 s.\nLast probability of success was %2.", _elapsedTime, _strPoS];
			};
	...
	if (_chatty && (_curAttempt == 0) && !(_stopHacking)) then {
		...
		systemChat "............";
		systemChat "** UNLUCKY **";
		systemChat "Challenged failed";
		_str = "last probability of success was ~" + str ([_p * 100, 2] call BIS_fnc_cutDecimals) + " %.";
		systemChat _str;
		systemChat "Same player shoot again ?";
		systemChat "............";
	}
